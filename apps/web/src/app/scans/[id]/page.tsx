...
function DeclarationEditor({declaration,disabled,save}:{declaration:Declaration;disabled:boolean;save:(text:string)=>Promise<void>}) {
  ...
  const inputId=`declaration-${declaration.id}`; const descriptionId=`${inputId}-source`;
  return <div className="declaration"><label htmlFor={inputId}><strong>{declaration.field.replaceAll("_"," ")}</strong><div id={descriptionId} className="muted small">{declaration.panel} · {Math.round((declaration.confidence ?? 0)*100)}%</div></label><input id={inputId} aria-describedby={descriptionId} className="input" value={text} disabled={disabled} onChange={(e)=>setText(e.target.value)} /><button className="button secondary small" disabled={disabled||!changed||!text.trim()} onClick={()=>save(text)}>Correct</button></div>;
}
